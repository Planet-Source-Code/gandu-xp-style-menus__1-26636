...
Attribute Click.VB_Description = "Raised when a non-VB menu item(i.e. one added using this control)  is clicked."
Public Event SystemMenuClick(ItemNumber As Long)
Attribute SystemMenuClick.VB_Description = "Raised when a system menu item is clicked."
Public Event ItemHighlight(ItemNumber As Long, bEnabled As Boolean, bSeparator As Boolean)
Attribute ItemHighlight.VB_Description = "Raised when a menu item is highlighted."
Public Event SystemMenuItemHighlight(ItemNumber As Long, bEnabled As Boolean, bSeparator As Boolean)
Attribute SystemMenuItemHighlight.VB_Description = "Raised when a system menu item is highlighted."
Public Event MenuExit()
Attribute MenuExit.VB_Description = "Raised when the user exits from a menu."
Public Event InitPopupMenu(ParentItemNumber As Long)
Attribute InitPopupMenu.VB_Description = "Raised just before a popup menu is about to be shown.  You can modify the items in this menu at this point."
Public Event WinIniChange()
Attribute WinIniChange.VB_Description = "Raised when system settings are changed."
Public Event NewMDIMenu()
Attribute NewMDIMenu.VB_Description = "Raised in an MDI project where child forms have their own menus.  When the child is shown, the control will detect the change in menu and raise this event."
...
Attribute RequestNewMenuDetails.VB_Description = "Raised if the control finds a new menu item after the CheckForNewMenuItems method has been called."
...
Public Function ShowPopupMenu( _
      ...
      ByVal x As Single, _
      ByVal y As Single, _
      Optional ByVal eOptions As CSPShowPopupMenuConstants = TPM_LEFTALIGN Or TPM_HORIZONTAL _
   ) As Long
Attribute ShowPopupMenu.VB_Description = "Shows the popup menu associated with a given menu item."
Dim lIndex As Long
Dim tP As POINTAPI
Dim tR As RECT
...
Attribute BackgroundPicture.VB_Description = "Sets a picture to tile into the background of the menu."
Dim tBm As BITMAP
   On Error Resume Next
...
Attribute ClearBackgroundPicture.VB_Description = "Removes the picture set with the BackgroundPicture property."
...
Attribute GetVersion.VB_Description = "Returns the current control version string."
   lMajor = App.Major
   lMinor = App.Minor
...
Attribute HighlightCheckedItems.VB_Description = "Gets/sets whether the background to icons for checked menu items are highlighted using a dither pattern when the menu item is selected."
...
Attribute MenuExists.VB_Description = "Returns whether a given menu index or key exists in the menu."
   MenuExists = (plMenuIndex(vKey) > 0)
End Property
Public Property Get MenuIndex(ByVal vKey As Variant) As Long
Attribute MenuIndex.VB_Description = "Gets the index of the menu item with the specified key."
Dim i As Long
   i = plMenuIndex(vKey)
   MenuIndex = i
   If (i < 0) Then
...
Attribute MenuKey.VB_Description = "Gets/sets a unique key string associated with a menu item."
...
Attribute MenuTag.VB_Description = "Gets/sets a string value associated with a menu item."
Dim lIndex As Long
...
Attribute MenuDefault.VB_Description = "Gets/sets whether the menu item should be shown in Bold font."
Dim lIndex As Long
   lIndex = MenuIndex(vKey)
   If (lIndex > 0) Then
...
Attribute TickIconIndex.VB_Description = "Gets/sets the 0 based index of an icon in the ImageList used to draw the check image for checked menu items if no other icon has been set."
...
Attribute SystemMenuCaption.VB_Description = "Gets/sets the caption of a system menu item given its ID."
...
Attribute SystemMenuCount.VB_Description = "Returns the number of items on the system menu."
Dim hSysMenu As Long
    hSysMenu = GetSystemMenu(m_hWndParent, 0)
    If (hSysMenu <> 0) Then
...
Attribute SystemMenuRemoveItem.VB_Description = "Removes an item from the system menu given its ID."
Dim hSysMenu As Long
    hSysMenu = GetSystemMenu(m_hWndParent, 0)
    If (hSysMenu <> 0) Then
...
Attribute SystemMenuAppendItem.VB_Description = "Appends an item to the System Menu and returns its identifier ID."
Dim hSysMenu As Long
    hSysMenu = GetSystemMenu(m_hWndParent, 0)
    If (hSysMenu <> 0) Then
        If (sCaption = "-") Then
...
Attribute SystemMenuRestore.VB_Description = "Restores the system menu to its default state."
...
Attribute ImageList.VB_Description = "Associates an ImageList with the control.  This can either be a COMCTL32 image list, a vbAccelerator ImageList or an API hImageList handle.  Make sure the ImageList contains some icons before using this."
    m_hIml = 0
...
Attribute Enabled.VB_Description = "Gets/sets whether a menu item appears enabled or not."
Dim tMII As MENUITEMINFO
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute Checked.VB_Description = "Gets/sets whether a menu item is checked or not."
Dim tMII As MENUITEMINFO
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute HelpText.VB_Description = "Gets/sets a string associated with a menu item.  This can be used to display help text in response to the MenuHighlight event."
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute ItemIcon.VB_Description = "Gets/sets the 0 based index of an icon in the ImageList to be shown against a menu item.  Set to -1 to show no icon."
...
Attribute hPopupMenu.VB_Description = "Returns a Win32 hMenu handle to a popup menu under the specified menu item."
Dim tMII As MENUITEMINFO
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute NextSibling.VB_Description = "Gets the next menu item from the specified menu item."
Dim lIndex As Long
...
Attribute SiblingCount.VB_Description = "Returns the number of menu items at the same level as the specified menu item."
Dim lIndex As Long
Dim lParentId As Long
Dim l As Long
Dim iCount As Long
...
Attribute HasChildren.VB_Description = "Returns whether the specified menu item has any sub-menus."
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute FirstChild.VB_Description = "Returns the index of the first menu item below the specified item."
Dim lIndex As Long
Dim lID As Long
Dim l As Long
    lIndex = MenuIndex(vKey)
...
Attribute Parent.VB_Description = "Gets the index of the parent menu item for a given menu item."
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute UltimateParent.VB_Description = "Gets the ultimate parent menu item index of a given menu item."
Dim lIndex As Long
Dim lR As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute IndexForMenuHierarchyParamArray.VB_Description = "Same as IndexForMenuHierarchy, but takes a parameter array rather than a long array type as the hierarchy."
Dim lH() As Long
Dim l As Long
...
Attribute IndexForMenuHierarchy.VB_Description = "Returns the index for the menu item which appears at the given hierarchy."
Dim l As Long
Dim lEnd As Long
...
Attribute GetHierarchyForIndexPosition.VB_Description = "Returns a hierarchy for a given menu index.  The hierarchy is an array indicating the position of the menu at each level."
Dim sItems() As String
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute HierarchyPath.VB_Description = "Returns a string containing a path equivalent of a given menu item.  For example, this might be Favourites\\VB Sites\\vbAccelerator."
Dim sItems() As String
Dim lH() As Long
Dim lItem As Long
Dim sOut As String
...
Attribute ClearSubMenusOfItem.VB_Description = "Removes all sub menus of a particular menu item."
Dim hMenu As Long
Dim iMenu As Long
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
         ...
         Else
            ...
            Else
               ...
               If (m_tMI(lIndex).lIconIndex > -1) Or (bChecked) Then
                  ...
                  End If
               End If
               If (m_hDCBack = 0) Or (bSelected) Or (bIsTopLevel) Then
               ...
               If ((bIsTopLevel) Or (m_eStyle = cspHighlightButton)) And (bSelected) Then
                  ...
                  Else
                     If (GetAsyncKeyState(vbKeyLButton) <> 0) And (m_tMI(lIndex).lActualID = m_tMI(lIndex).lID) Then
                        ...
                        pGetTextPosition lHDC, lIndex, False, tOB
                        pDrawMenuCaption lHDC, lIndex, tDI.rcItem
                     Else
                        DrawEdge lHDC, tOB, EDGE_ETCHED2, BF_RECT
...
Attribute IDForIndex.VB_Description = "Returns the Win32 Menu Identifier for a given menu item."
Dim lIndex As Long
    lIndex = MenuIndex(vKey)
    If (lIndex > 0) Then
...
Attribute AddItem.VB_Description = "Adds a new menu item and optionally sets extended properties."
Dim lID As Long
...
Attribute ReplaceItem.VB_Description = "Replaces a menu item with the specified properties."
Dim lIndex As Long
Dim sItems() As String
Dim lH() As Long
Dim lR As Long
...
Attribute InsertItem.VB_Description = "Same as the AddItem method, except the menu item is inserted before the specified menu item."
...
Attribute SubClassMenu.VB_Description = "Initialises the icon menu against a form's menu.  Call this method before using any other methods except ImageList."
Dim hMenu As Long
...
Attribute CheckForNewItems.VB_Description = "Interogates the menu and checks for any new items or removed items (i.e. ones which have been Loaded, Unloaded or had their Visible property changed).  For each new item found, the RequestNewMenuDetails event is raised."
Dim i As Long
...
Attribute MenuItemsPerScreen.VB_Description = "Returns the number of menu items which can be fitted vertically on the screen."
Dim tWR As RECT
Dim lR As Long
